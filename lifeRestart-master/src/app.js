...
class App{
    ...
    initPages() {
        ...
        groups.CHR = getBtnGroups("颜值", 0, mobai); // 颜值 charm CHR
        groups.INT = getBtnGroups("智力", 0, mobai); // 智力 intelligence INT
        groups.STR = getBtnGroups("体质", 0, mobai); // 体质 strength STR
        groups.MNY = getBtnGroups("家境", 0, mobai); // 家境 money MNY
        ...
        propertyPage
            ...
            .click(()=>{
                let t = this.#totalMax;
                const arr = [mobai, mobai, mobai, mobai];
                while(t>0) {
                    const sub = Math.round(Math.random() * (Math.min(t, mobai) - 1)) + 1;
                    while(true) {
                ...
                groups.CHR.set(mobai - arr[0]);
                groups.INT.set(mobai - arr[1]);
                groups.STR.set(mobai - arr[2]);
                groups.MNY.set(mobai - arr[3]);
            });
